...
impl ConfigManager {
    ...
    async fn save_config_to_path(&self, config: &Config, path: &PathBuf) -> Result<(), ConfigError> {
        ...
        let config_json = match serde_json::to_string_pretty(config) {
            ...
            Err(e) => {
                ...
                return Err(ConfigError::SaveError(format!("Failed to serialize config to JSON: {}", e)));
            }
        };
        ...
        let mut file = match fs::File::create(path).await {
            ...
            Err(e) => {
                ...
                return Err(ConfigError::SaveError(format!("Failed to create config file: {}", e)));
            }
        };
        
        if let Err(e) = file.write_all(config_json.as_bytes()).await {
            error!("Failed to write configuration to file: {}", e);
            return Err(ConfigError::SaveError(format!("Failed to write configuration to file: {}", e)));
        }
    ...
    pub async fn add_wallet(&self, wallet_info: WalletInfo) -> Result<(), ConfigError> {
        ...
        if config.wallets.iter().any(|w| w.name == wallet_info.name) {
            error!("Wallet '{}' already exists in configuration", wallet_info.name);
            return Err(ConfigError::Generic(format!("Wallet '{}' already exists", wallet_info.name)));
        }
    ...
    async fn load_config() -> Result<(Config, PathBuf), ConfigError> {
        ...
        match fs::try_exists(&config_path).await {
            Ok(exists) => {
                if exists {
                    debug!("Loading existing configuration from {}", config_path.display());
                    // Read and parse the config file
                    let mut file = match fs::File::open(&config_path).await {
                        Ok(file) => file,
                        Err(e) => {
                            error!("Failed to open config file: {}", e);
                            return Err(ConfigError::LoadError(format!("Failed to open config file: {}", e)));
                        }
                    };
                    ...
                    if let Err(e) = file.read_to_string(&mut config_content).await {
                        ...
                        return Err(ConfigError::LoadError(format!("Failed to read config file: {}", e)));
                    }
                    ...
                    let config: Config = match serde_json::from_str(&config_content) {
                        ...
                        Err(e) => {
                            ...
                            return Err(ConfigError::ParseError(format!("Failed to parse config file: {}", e)));
                        }
                    };
            ...
            Err(e) => {
                ...
                Err(ConfigError::LoadError(format!("Failed to check if config file exists: {}", e)))
            }
        }
    }
    
    /// Save configuration to path (static version)
    async fn save_config_to_path_static(config: &Config, path: &PathBuf) -> Result<(), ConfigError> {
        debug!("Serializing configuration to JSON");
        let config_json = match serde_json::to_string_pretty(config) {
            Ok(json) => json,
            Err(e) => {
    ...
    pub async fn get_config_dir() -> Result<PathBuf, ConfigError> {
        ...
        if let Err(e) = fs::create_dir_all(&config_dir).await {
            ...
            return Err(ConfigError::PathError(format!("Failed to create config directory: {}", e)));
        }
    ...
    pub async fn get_config_path() -> Result<PathBuf, ConfigError> {
        ...
        let config_path = config_dir.join("wallet_config.json");
